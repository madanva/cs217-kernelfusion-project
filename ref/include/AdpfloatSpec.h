...
namespace spec {
  namespace Adpfloat {
    ...
    const bool ReserveZero =
        1; // if not using denormal, reserve 00....0, 10....0 as zero
  } // namespace Adpfloat
} // namespace spec
...
// XXX: both 1000,0000 or 0000,0000 stands for zero, might implement signed zero
// later
...
public:
  ...
  NVUINTW(width) to_rawbits() const { return TypeToNVUINT(*this); }
  ...
  ac_float<M + 2, 2, E + 2, AC_RND> to_ac_float(
      AdpfloatBiasType adpfloat_bias = 0) const {
    if (spec::Adpfloat::ReserveZero && is_zero())
      return 0;
  ...
  void set_value_ac_float(
      ac_float<M + 2, 2, E + 2, AC_RND> in_ac_float,
      AdpfloatBiasType adpfloat_bias = 0) {
    NVINTW(E + 2) exp_tmp;
  ...
  float max_value(AdpfloatBiasType adpfloat_bias = 0) const {
    // max mantissa
    float max_exp =
        pow(2, exp.width) - 1 + (int)adpfloat_bias + spec::kAdpfloatOffset;
  ...
  void set_value_fixed(
      const NVINTW(W1) in, const AdpfloatBiasType adpfloat_bias = 0) {
...
inline bool operator==(
    const AdpfloatType<W, E>& lhs, const AdpfloatType<W, E>& rhs) {
  bool is_equal = true;
...
inline std::ostream& operator<<(ostream& os, const AdpfloatType<W, E>& adpfloat) {
  os << "<sign, exp, man> <" << adpfloat.sign << " " << adpfloat.exp << " "
     << adpfloat.man << "> ";
