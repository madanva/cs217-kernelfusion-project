/*
 * Attention Unit Testbench (Fully Fused Lean — Single Thread)
 *
 * Same algorithm as FullyFused but single-thread (no ac_channel overhead).
 * Only writes N output vectors to mem3 (zero intermediate materialization).
 * All N writes are checked against golden reference.
 */
...
#include "Spec.h"
#include "helper.h"
#include "AttnFullyFusedLean.h"

#define NVHLS_VERIFY_BLOCKS (AttnFullyFusedLean)
    ...
    while (1) {
      ...
      if (large_req.PopNB(req)) {
        if (req.is_write) {
          ...
          // All writes should be final output (no intermediates)
          if (mem_idx == 3) {
            NVUINT8 row = ts_idx;
...
SC_MODULE(testbench) {
  ...
  NVHLS_DESIGN(AttnFullyFusedLean) dut;
  Source source;
  Dest dest;
