/*
 * Attention Unit Testbench (Unfused)
 *
 ...
 * - Simulate GBCore storage for intermediate QKT scores and softmax weights
 ...
 *
 * The unfused variant writes 3 stages to GBCore (mem 3):
 *   Stage 1: N writes (QKT scores)
 *   Stage 2: N writes (softmax weights)
 *   Stage 3: N writes (final output) — checked against golden
 */
...
#include "Spec.h"
#include "helper.h"
#include "AttnUnfused.h"

#define NVHLS_VERIFY_BLOCKS (AttnUnfused)
...
// =============================================================================
// Dest Module
// Simulates GBCore storage for the unfused variant.
// Unfused writes 3N vectors to mem3: N QKT + N softmax + N output.
      ...
      if (large_req.PopNB(req)) {
        if (req.is_write) {
          ...
          if (mem_idx == 3) {
            ...
            // Unfused: 3 stages, each writes N×ceil(N/16) vectors (QKT, softmax)
            // plus N×1 (output). Check only the last N writes (Stage 3 output).
            static const int kNumVecsPerRow = (kSeqLen + spec::kVectorSize - 1) / spec::kVectorSize;
            // Stage 1: N*kNumVecsPerRow writes, Stage 2: N*kNumVecsPerRow writes,
            // Stage 3: N writes. Check writes after first 2*N*kNumVecsPerRow.
            if (total_writes > 2 * kSeqLen * kNumVecsPerRow) {
              NVUINT8 row = ts_idx;
