/*
 * Attention Unit Testbench (Partially Fused)
 *
 ...
 * - Simulate GBCore storage for intermediate attention weights
 * - Compare final output against double-precision golden reference
 * - Read back performance counters (cycle_count, gb_read_count, gb_write_count)
 *
 * The partial fused variant writes 2 stages to GBCore (mem 3):
 *   Stage 1: N writes (fused QKT+softmax attention weights)
 *   Stage 2: N writes (final output) — checked against golden
 */
...
#include "Spec.h"
#include "helper.h"
#include "AttnPartialFused.h"

#define NVHLS_VERIFY_BLOCKS (AttnPartialFused)
...
SC_MODULE(Source) {
  ...
  void run() {
    ...
    // Wait for computation, then read performance counters
    // N=64 needs ~130k cycles at II=3
    wait(200000);
...
// =============================================================================
// Dest Module
// Simulates GBCore storage for the partial fused variant.
// Partial fused writes 2N vectors to mem3: N attn weights + N output.
...
SC_MODULE(Dest) {
  ...
  // GBCore storage for intermediate results (N timesteps × ceil(N/16) vectors)
  ...
  void run() {
    ...
    while (1) {
      ...
      if (large_req.PopNB(req)) {
        if (req.is_write) {
          if (req.memory_index == 3) {
            NVUINT16 ts = req.timestep_index;
            NVUINT8 vi = req.vector_index;
            gbcore_storage[ts][vi] = req.write_data;

            cout << sc_time_stamp() << " Write to mem3: ts=" << (int)ts
                 << " vec_idx=" << (int)vi
                 << " total_writes=" << total_writes << endl;

            // Stage 1 writes ceil(N/kVectorSize) vectors per row × N rows
            // Stage 2 writes 1 vector per row × N rows (final output)
            static const int kNumVecsPerRow = (kSeqLen + spec::kVectorSize - 1) / spec::kVectorSize;
            if (total_writes >= kSeqLen * kNumVecsPerRow) {
              NVUINT8 row = ts;
          ...
          // Read request - respond with Q/K/V or GBCore storage
...
SC_MODULE(testbench) {
  ...
  NVHLS_DESIGN(AttnPartialFused) dut;
  Source source;
  Dest dest;
